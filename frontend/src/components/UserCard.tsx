...
import { Candidates } from "../firebase/auth/models/User";
import { useAppDispatch, useAppSelector } from "../hooks/useRedux";
import { setDetailCandidate } from "../store/main-store/main.slice";

const UserCard = ({ user }: { user: Candidates }) => {
  ...
  const handleChangePage = () => {
    ...
    dispatch(setDetailCandidate(user));
      ...
      <div className="rounded shadow-md p-5 border-[0.2px] border-color-text border-opacity-10">
        <div className="flex gap-3">
          ...
          <div className="flex flex-col w-full gap-1">
            <div className="flex justify-between   w-auto mb-2">
              <h1
            ...
            <p className="text-gray flex items-center text-color-text opacity-80 gap-1 flex-wrap">
              <span className="flex gap-2 mr-2">
                <MapPin className="w-4 text-gray-body" /> {user?.location},{" "}
                {user?.province}
              </span>
              <span className="flex gap-2 ml-2">
                {new Intl.NumberFormat("id-ID", {
                  style: "currency",
                  currency: "IDR",
                }).format(user?.minimumSalary)}{" "}
                -{" "}
                {new Intl.NumberFormat("id-ID", {
                  style: "currency",
                  currency: "IDR",
                }).format(user?.maximumSalary)}
              </span>
            </p>

            <p className="text-gray-body text-sm mt-2">{user?.description}</p>
